...
## ✅ LIVE PROOF (2026-06-08) — see `35-MVP-PROOF.md`

The MVP flow was proven end-to-end through the UI against the running cluster: a UI-created data
source (`mvp35-proof`, id `6a270d5b3c7dae70f384bac0`) ran FileDiscovery → FileProcessor → Validation
(3/3 valid) → **S3 output write**, producing `s3://ez-phase34-output/output_20260608190334.{format}`
(content = the CSV-derived records). This verifies the output path: **G5, G7, G8, G9, G10, G11** are
proven live (FIX-IN-CODE → effectively VERIFIED). Remaining: G12 (minor polish — keyPrefix not in the
written key, `{format}` literal, CSV vs JSON output), and live re-verify of G3 (schema-edit survival)
and G4 (console-port guard) which were not re-exercised in this session.
...
| G9 | BLOCKER | FIX-IN-CODE | FE output type casing | **THE actual root cause of "output bucket/prefix not saved".** `DestinationEditorModal.handleSubmit` built the type-specific config (`s3Config`, `kafkaConfig`, …) by comparing `values.type` against **lowercase** literals (`=== 's3'`), but the protocol `<Select>` emits **UPPERCASE** values (`'S3'`, `'Kafka'`, `'FTP'`, `'SFTP'`, `'HTTP'`). So the S3 branch never matched → `s3Config` was **never built or sent** → backend fell back to the server bucket with `KeyPrefix=null` and the typed prefix was silently dropped. Also made the OutputTab config-column case-insensitive (S3 rows showed "לא מוגדר"). The 35-02 unit test masked this by simulating the branch with lowercase `'s3'`. **Fixed (35-03 loop):** compare on a lowercased `typeKey` in `handleSubmit`; case-insensitive S3 check in `OutputTab`. Found by orchestrator driving the live UI at the 35-03 checkpoint. | 35-03 (loop) |
| G10 | BLOCKER | FIX-IN-CODE | FE request mapping drops S3Config | **THE persistence gap.** Even after G9 builds `s3Config` in form state, the create/update request builders (`DataSourceFormEnhanced` + `DataSourceEditEnhanced`, the `Destinations.map`) mapped `KafkaConfig/FolderConfig/SftpConfig/HttpConfig` but **omitted `S3Config`** — so the bucket/prefix was dropped on the wire and the backend fell back to the server bucket with `KeyPrefix=null`. Verified live: UI table showed `ez-phase34-output/processed` but Mongo persisted `Bucket=ez-phase34-input, KeyPrefix=null`. Also fixed the two `prepareCloneData` spots (clone dropped S3Config). **Fixed (35-03 loop):** added `S3Config: d.s3Config` to both request mappings + `s3Config` to clone. Twin of G7 (load side). Found by orchestrator driving the live create flow + Mongo verify. | 35-03 (loop) |
| G12 | MINOR | OPEN | BE S3 output key construction | Non-blocking polish found during the live proof: `S3OutputHandler` writes the object at the bucket root (`output_20260608190334.{format}`) instead of under the persisted `KeyPrefix` (`processed/`), and does not substitute the `{format}` filename placeholder. Also the destination `OutputFormat` defaulted to `csv` so the object is CSV, not the JSON array the runbook anticipated. Core MVP (object lands in `ez-phase34-output`) is unaffected. Closing plan: small `S3OutputHandler` key-construction fix + format-default. | 35-05 or follow-up |
| G11 | BLOCKER | FIX-IN-CODE | BE output handler dispatch casing | **THE final pipeline blocker.** With G8/G10 fixed the run reached the output stage (validation Success, 3 valid records) but the Output service threw `No handler found for destination type: S3` and silently dropped the records — no MinIO object. Root cause (same class as G9): `ValidationCompletedEventConsumer` dispatched via `h.CanHandle(destination.Type)` where `destination.Type` is persisted in the Select's case (`"S3"`), but every `IOutputHandler.CanHandle` compares to a lowercase literal (`== "s3"`), so `"S3" != "s3"`. **Fixed (35-03 loop):** normalize `destination.Type` to lower-invariant before dispatch (fixes S3 and all types). Found by orchestrator tracing the live run in the output-service logs. Requires Output-service rebuild. | 35-03 (loop) |
